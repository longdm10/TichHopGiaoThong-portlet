...
<object id="plugin0" type="application/x-cryptolib05plugin" width="0"
	height="0"></object>
<div class="bgtlemain">
	<%
...
<div class="wd-main-content wd-page-staff">
	<div id="Accordion1" class="Accordion" tabindex="0">
		<div class="AccordionPanel">
			<div class="AccordionPanelTab">
				<h4>
					Thông tin chung<img
						src="<%=request.getContextPath()%>/img/front/next_tle.png">
				</h4>
			...
			<div class="AccordionPanelContent">
				...
				<div class="tbl_left">
					<table class="table_noboder">
						<thead>
							<tr>
								<th class="text-left" width="150px"><liferay-ui:message
										key="vn.dtt.kehoach.mabankhai" /></th>
								<td class="text-left"><%=temp.getDocumentName()%></td>
							</tr>
							<tr>
								<th class="text-left"><liferay-ui:message
										key="vn.dtt.kehoach.quoctich" /></th>
							...
							<tr>
								...
								%>
								<th class="text-left"><liferay-ui:message
										key="vn.dtt.kehoach.thoigiandenroitu" /></th>
								...
								%>
								<th class="text-left"><liferay-ui:message
										key="vn.dtt.kehoach.thoigiandenroitoi" /></th>
				...
				<div class="tbl_left">
					<table class="table_noboder">
						<thead>
							<tr>
								<th class="text-left" width="150px"><liferay-ui:message
										key="vn.dtt.kehoach.loaibankhai" /></th>
		...
		<form id="userForm" name="userForm" method="post">
			...
			<%@ include
				file="/html/kehoach/thong_tin_chung_ho_so_lich_su_ho_so.jsp"%>

			<div class="AccordionPanel">
				<div class="AccordionPanelTab">
					<h4>
						Nội dung giấy tờ<img
							src="<%=request.getContextPath()%>/img/front/next_tle.png">
					</h4>
					<div class="righttle"></div>
				</div>
	...
	<div class="boxbt">
		...
		<%
			if(messageType.compareTo(String.valueOf(PageType.THANH_PHAN_HO_SO)) != 0) {
		%>
		<button id="btnTiepNhan" class="bt" type="button"
			onclick="if(validatorCLDD()) summitForm('<%=actionUrlTiepNhan.toString()%>')">
			<i class="tiepnhan"></i>Cấp lệnh điều động
		</button>
		<%
			}
		%>
		...
		<button class="bt" type="button"
			onclick="displayInputRejectAndChangeAction('<%=actionUrlChuyenGiaoDien.toString()%>')">
			<i class="tiepnhan"></i>Cấp lại lệnh điều động
		</button>
		<button class="bt" type="button"
			onclick="displayInputRejectAndChangeAction('<%=actionUrlHuy.toString()%>')">
			<i class="tiepnhan"></i>Hủy lệnh điều động
		</button>
		<%
			}
		%>
		...
		<div id="duyetkylenhdieudong">
			<button class="bt" type="button"
				onclick="summitForm('<%=actionUrlDuyet.toString()%>')">
				<i class="tiepnhan"></i>Duyệt lệnh điều động
			</button>
			<button class="bt" type="button" id="showdatakyso">
				<i class="tiepnhan"></i>Ký số BCY
			</button>
			<button class="bt" type="button"
				onclick="summitForm('<%=actionUrlChuyenGiaoDien.toString()%>')">
				<i class="tiepnhan"></i>Sửa lệnh điều động
			</button>
		</div>
		...
		%>
		<input type="hidden" id="satateBanKhaiAnNinh"
			value="<%=satateBanKhaiAnNinh%>" /> <input type="hidden"
			id="satateThongBao" value="<%=satateThongBao%>" /> <input
			type="hidden" id="loaiThuTuc" value="<%=loaiThuTuc%>" />

		<button class="bt" type="button"
			onclick="if(validatorChapNhanHoSo()) summitForm('<%=actionUrlTiepNhan.toString()%>')">
			<i class="tiepnhan"></i>Thông báo chấp thuận
		</button>
		<button class="bt" type="button" onclick="displayInputReject();">
			<i class="chuyen"></i>Thông báo từ chối
		</button>

		<!-- button sua doi bo xung, luon luon xuat hien o? moi truong hop, thong bao tu can bo hai quan toi nguoi dan -->
		<button id="btnTiepNhan" class="bt" type="button"
			onclick="displayInputModify();">
			<i class="tiepnhan"></i>Thông báo hướng dẫn DN bổ sung
		</button>
		...
		<button class="bt" type="button"
			onclick="summitForm('<%=actionUrlTiepNhan.toString()%>')">
			<i class="tiepnhan"></i>Tiếp nhận bản khai
		</button>
		<button class="bt" type="button" onclick="displayInputReject();">
			<i class="chuyen"></i>Từ chối bản khai
		</button>
		...
		<div id="duyetkylenhdieudong">
			...
			%>
			<button class="bt" type="button"
				onclick="getFileComputerHashDongDau('<%=actionUrlDongDau.toString()%>')">
				<i class="tiepnhan"></i>Đóng dấu BCY
			</button>

			<input type="hidden" id="RequestCode" name="RequestCode"
				value="<%=issueShiftingOrderTest.getAttachedFile()%>">
		</div>
		<%
			}
		...
		<div class="tbl_left boxlabel" id="kstable" style="width: 49%">
			<table class="table_noboder">
				<thead>
					<tr>
						<th class="text-left" width="30%">Người ký&nbsp;<span
							style="color: #FF0000;">(*)</span>
						</th>
						<td class="text-leftu"><input type="text"
							class="egov-inputfield" style="width: 98%; text-transform: none;"
							id="signName" name="signName"
							value="<%=Validator.isNotNull(shiftingOrderEdit) ? shiftingOrderEdit.getSignName() : ""%>">
						</td>
					</tr>
					<tr>
						<th class="text-left" width="30%">Ngày ký&nbsp;<span
							style="color: #FF0000;">(*)</span></th>
						<td class="text-leftu"><input type="text" style="width: 91%"
							name="signDate" id="signDate"
							value="<%=Validator.isNotNull(shiftingOrderEdit)?FormatData.parseDateToTringType3(shiftingOrderEdit.getSignDate()):""%>"
							onclick="gtCalendar('signDate')"> <img alt=""
							src="<%=request.getContextPath()%>/img/front/galden.png"
							onclick="gtCalendar('signDate')"></td>
					</tr>
					<tr>
						<th class="text-left" width="30%">Địa điểm ký&nbsp;<span
							style="color: #FF0000;">(*)</span></th>
						<td class="text-leftu"><input type="text"
							class="egov-inputfield" style="width: 98%" id="signPlace"
							name="signPlace" value="<%=dmHisMaritime.getCitycode()%>">
						</td>
					...
					<tr>
						...
						<td style="text-align: center;">
							...
							<button class="bt" type="button"
								onclick="if(validatorTTKS()) getFileComputerHash('<%=actionUrlKyso.toString()%>')">
								<i class="tiepnhan"></i>Thực hiện
							</button>
							<button class="bt" type="button" id="quaylai">
								<i class="tiepnhan"></i>Quay lại
							</button>
						</td>
					</tr>
		...
		<div align="left" id="modify_data" style="display: none;">
			...
			<table style="width: 60%">
				<tr>
					<td style="width: 100%">Nhập lý do&nbsp;<span
						style="color: #FF0000;">(*)</span>:
					</td>
				</tr>
				<tr>
					<td style="width: 100%" class="text-left"><textarea
							id="ly_do_suadoi_bosung" name="<%=PageType.LY_DO_SUADOI_BOSUNG%>"
							rows="4" cols="100" maxlength="3000"></textarea></td>
				</tr>
				<tr>
					<td style="text-align: left;">
						<button class="bt" type="button"
							onclick="if(validatorModify()) summitForm('<%=actionUrlSuaDoiBoXung.toString()%>')">
		...
		<div align="left" id="reject_data" style="display: none;">
			<br>
			<table style="width: 60%">
				<tr>
					<td style="width: 100%">Nhập lý do&nbsp;<span
				...
				<tr>
					<td style="width: 100%" class="text-left"><textarea
							id="LY_DO_TU_CHOI" name="<%=PageType.LY_DO_TU_CHOI%>" rows="4"
							cols="100" maxlength="3000"></textarea></td>
				</tr>
				<tr>
					<td style="text-align: left;">
						<button class="bt" type="button"
							onclick="if(validatorXacNhanTuChoi()) summitForm('<%=actionUrlTuChoi.toString()%>')">
							<i class="tiepnhan"></i>Xác nhận
						</button>
					</td>
				</tr>
...
<div>
	<applet name='caApplet' id='caApplet' width="0" height="0"
		...
		codebase='/TichHopGiaoThong-portlet/applet/'> </applet>
</div>
...
function getFileComputerHashDongDau(urlFromSubmit) {
					...
					for (var i = 0; i < hashComputers.length; i++) {
						...
						if (plugin().valid) {
							...
							if (msg == 'success') {
								...
								if (code === 0 || code === 7) {
									var sign = plugin().Signature;
									completeSignatureDongDau(sign,
											signFieldName, filePath,
											urlFromSubmit);
	...
	function completeSignatureDongDau(sign, signFieldName, filePath,
			urlFromSubmit) {
		//	alert("--------- vao day completeSignature--------------");
		var msg = '';
		$
						...
						if (data) {
							...
							if (msg === 'success') {
								...
								var urlAction = document
										.getElementById('userForm').action;
								if (urlAction == '') {
									document.getElementById('userForm').action = urlFromSubmit;
								}
								document.getElementById('userForm').submit();
	...
	function validatorCLDD() {
		...
		if (dataCertificateNo.length == 0) {
			...
			//	return false;
		} else if (holdPortHarbourCode === ''
				|| typeof holdPortHarbourCode === 'undefined') {
			alert("Đề nghị chọn bến cảng (Xác báo không có thông tin bến cảng) !");
			return false;
		} else if (holdPortWharfCode === ''
				|| typeof holdPortWharfCode === 'undefined') {
