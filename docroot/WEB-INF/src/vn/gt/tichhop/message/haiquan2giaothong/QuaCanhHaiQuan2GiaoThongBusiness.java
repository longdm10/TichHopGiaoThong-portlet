...
import vn.gt.utils.document.ChuKiSoGiaoThongUtil;
		...
		try {
			...
			if (requestCode != null) {
				...
				if (actionType == PageType.ACTION_TYPE_TU_CHOI) {
					...
					Header header = BusinessUtils.crateHeader(tempDocument, MessageType.QUA_CANH, function, messageType, userName, interfaceRequest);
		...
		switch (actionType) {
			...
			case PageType.ACTION_TYPE_KYSODUYET:
				...
				if (Validator.isNotNull(perForTransitDuyetkyso)) {
					...
					perForTransitDuyetkyso.setRequestState(TrangThaiBanKhaiQuaCanh.CHAP_NHAN_BAN_KHAI);
				//	perForTransitDuyetkyso.setIsApproval(PageType.DUYET_PHE_CHUAN);
					...
					perForTransitDuyetkyso.setSignTitle(perForTransitDuyetkyso.getRepresentative());
...
//					log.info("GUI=====GIAY_PHEP_QUA_CANH=====requestCode====" + requestCodeGiayPhepQuaCanh);
	...
	private void keHoachXuLyLenhDieuDong(TempDocument document, int actionType, ActionRequest request, ActionResponse httpReq, String userName,
...
//							}
	...
	private void thuTucTaoGiayPhepQuaCanh(long documentName, int messageType, int documentYear, int actionType, String userName, String requestCode,
			ActionRequest request, ActionResponse resqonse) {
		
		try {
			
			log.info("===vao=taoGiayPhepQuaCanhThuTuc=======documentName=" + documentName);
			// Giay phep roi cang so
			String numberPermissionForTransit = ParamUtil.getString(request, "numberPermissionForTransit");
			
			String portofAuthority = ParamUtil.getString(request, "portofAuthority");
			
			String nameOfShip = ParamUtil.getString(request, "nameOfship");
			String flagStateOfShip = ParamUtil.getString(request, "stateCode");
			double gt = ParamUtil.getDouble(request, "grt");
			int numberOfCrews = ParamUtil.getInteger(request, "numberOfCrews");
			int numberOfPassengers = ParamUtil.getInteger(request, "passengerNumber");
			String callSign = ParamUtil.getString(request, "callSign");
			String nameOfMaster = ParamUtil.getString(request, "nameOfMaster");
			
			String transitCargo = ParamUtil.getString(request, "transitOfCargo");
			double volumeCargo = ParamUtil.getDouble(request, "volumeTransitCargo");
			String cargoUnit = ParamUtil.getString(request, "unitVolumeTransitCargo");
			
			String permittedTransitFrom = ParamUtil.getString(request, "permittedTransitFrom");
			String permittedTransitTo = ParamUtil.getString(request, "permittedTransitTo");
			
			Date timeOfDeparture = ParamUtil.getDate(request, "timeOfDeparture", FormatData.formatDateShort3);
			Date validUntil = ParamUtil.getDate(request, "validUntil", FormatData.formatDateShort3);
			Date dateOfSign = ParamUtil.getDate(request, "dateOfSign", FormatData.formatDateShort3);
			...
			// Date createdDate = ParamUtil.getDate(resourceRequest, param,
			// FormatData.formatDateShort3);
			Date createdDate = new Date();
			
			String certificateNo = ParamUtil.getString(request, "certificateNo");
			
			String unitCertificateNo = ParamUtil.getString(request, "unitCertificateNo");
			
			String requestCodePerForTransit = ParamUtil.getString(request, "requestCodePerForTransit");
			
			long idPerForTransit = ParamUtil.getLong(request, "idPerForTransit");
			String versionNo = ParamUtil.getString(request, "versionNo");
			String representative = ParamUtil.getString(request, "representative");
			String remarks = ParamUtil.getString(request, PageType.LY_DO_TU_CHOI);
			...
			// TODO Qua Canh truong hop duyet
			// perForTransit.setIsApproval(isApproval);
			// perForTransit.setApprovalDate(approvalDate);
			// perForTransit.setApprovalName(approvalName);
			
			// TODO Qua Canh truong hop cap lai
			// perForTransit.setRemarks(remarks);
			
			// TODO Qua Canh truong hop huy?
			// perForTransit.setIsCancel(isCancel);
			// perForTransit.setCancelDate(cancelDate);
			// perForTransit.setCancelName(cancelName);
			// perForTransit.setCancelNote(cancelNote);
			...
			String suaPerForTransit = ParamUtil.getString(request, PageType.LAN_SUA_PERMISSION_FOR_TRANSIT);
	...
	public boolean thuTucSendMessageGiayPhepQuaCanh(int messageType, String versionPortClean, int actionType, TempDocument tempDocument, String userName,
		...
		try {
			...
			log.info("====sendMessageGiayPhepQuaCanh==versionPortClean==" + versionPortClean);
			String xmlData = "";
			...
			if (requestCodeGiayPhepQuaCanh != null) {
				...
				if (actionType == PageType.ACTION_TYPE_HUY) {
					...
					log.info("====ACTION_TYPE_DUYET" + "===version====" + versionPortClean);
					String function = MessageType.FUNCTION_KHAI_BAO;
					
					Header header = BusinessUtils.crateHeader(tempDocument, MessageType.QUA_CANH, function, messageType, userName, interfaceRequest);
					header.getReference().setVersion(versionPortClean);
					
					xmlData = businessUtils.sendMessageResult(header, versionPortClean, tempDocument.getRequestCode());
	...
	public boolean sendMessageLenhDieuDong(int actionType, TempDocument document, String userName, String requestCodeShiftOrder,
		...
		boolean result = true;
		try {
			log.info("sendMessageLenhDieuDong==Giay phep qua canh==" + MessageType.LENH_DIEU_DONG + "==actionType==" + actionType + "==requestCodeShiftOrder==" + requestCodeShiftOrder);
			...
			if (requestCodeShiftOrder != null) {
				...
				if (actionType == PageType.ACTION_TYPE_TU_CHOI) {
					String function = MessageType.FUNCTION_TU_CHOI_HO_SO_CHUNG_TU;
					Header header = BusinessUtils.crateHeader(document, MessageType.QUA_CANH, function, MessageType.LENH_DIEU_DONG, userName,
							interfaceDocument);
					...
					Header header = BusinessUtils.crateHeader(document, MessageType.QUA_CANH, MessageType.FUNCTION_HUY_LENH_DIEU_DONG,
	...
	public void sendMessageTuChoiHoSo(String userName, ActionRequest request, TempDocument document, String requestCode, String lyDoTuChoi) throws RemoteException {
		...
		String function = MessageType.FUNCTION_TU_CHOI_HO_SO_CHUNG_TU;
		Header header = BusinessUtils.crateHeader(document, MessageType.QUA_CANH, function, MessageType.HO_SO, userName, interfaceRequest);
	...
	public void sendMessageChapNhan(int messageType, String function, TempDocument document, String userName, String requestCode, ActionRequest request) {
		try {
			...
			Header header = BusinessUtils.crateHeader(document, MessageType.QUA_CANH, function, messageType, userName, interfaceRequest);
	...
	private String createXmlHuyHoSo(int messageType, String xmlData, BusinessUtils businessUtils, TempDocument tempDocument, String userName,
		...
		try {
			...
			if (messageType == MessageType.XAC_BAO_TAU_QUA_CANH) {
				...
				Header header = BusinessUtils.crateHeader(tempDocument, MessageType.QUA_CANH, function, MessageType.THONG_BAO_HUY_XAC_BAO, userName, interfaceRequest);
				...
				Header header = BusinessUtils.crateHeader(tempDocument, MessageType.QUA_CANH, MessageType.FUNCTION_HUY_LENH_DIEU_DONG,
				...
				Header header = BusinessUtils.crateHeader(tempDocument, MessageType.QUA_CANH, function, MessageType.HO_SO, userName, interfaceRequest);
