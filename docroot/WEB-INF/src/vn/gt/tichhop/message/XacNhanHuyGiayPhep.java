...
public class XacNhanHuyGiayPhep {
	...
	static Log _log = LogFactoryUtil.getLog(XacNhanHuyGiayPhep.class);
	...
	public String huyGiayPhepRoiCang(Header header, String dataInput, String data) {
		...
		try {
			...
			if (obj instanceof XacNhanHuyThuTuc) {
				...
				businessUtils.sendMessageHuyGiayPhepRoiCang(header, "Bo Giao Thong Van Tai", "Thu Tuc", header.getFrom().getName(), "System", new Date(),new Integer(((XacNhanHuyLenhDieuDong) obj).getIsApprove()), null);
			}
	...
	public String huyGiayPhepQuaCanh(Header header, String dataInput, String data) {
		...
		try {
			...
			if (obj instanceof XacNhanHuyLenhDieuDong) {
				...
				header.setSubject(subject);
				businessUtils.sendMessageHuyGiayPhepQuaCanh(header, "Bo Giao Thong Van Tai", "Thu Tuc", header.getFrom().getName(), "System", new Date(), new Integer(((XacNhanHuyLenhDieuDong) obj).getIsApprove()), null);
			}
			// Cap nhat trang thai la tiep nhan voi truong hop da tiep nhan ban khai.
			if (header != null && header.getReference() != null) {
				businessUtils.updateHistory(header.getReference().getMessageId(), TrangThaiHoSo.HISTORY_REQUEST_STATE_GHI_NHAN);
	...
	public String khaiHuyHoSo(Header header, String dataInput, String data) {
		...
		try {
			...
			if (obj instanceof XacNhanHuyLenhDieuDong) {
				...
				header.setSubject(subject);
				businessUtils.sendMessageKhaiHuyHoSo(header, "Bo Giao Thong Van Tai", "Thu Tuc", header.getFrom().getName(), "System", new Date(), new Integer(((XacNhanHuyLenhDieuDong) obj).getIsApprove()), null);
			}
			// Cap nhat trang thai la tiep nhan voi truong hop da tiep nhan ban khai.
			if (header != null && header.getReference() != null) {
				businessUtils.updateHistory(header.getReference().getMessageId(), TrangThaiHoSo.HISTORY_REQUEST_STATE_GHI_NHAN);
