...
public class TempAnimalQuarantineFinderImpl extends BasePersistenceImpl<TempAnimalQuarantine> implements TempAnimalQuarantineFinder {
	...
	public TempAnimalQuarantine getLastByDocumentNameAndDocumentYear(long documentName, int documentYear) {
		...
		/*	interfaceRequest.append("SELECT inter.RequestCode FROM interface_request inter INNER JOIN (");
			interfaceRequest.append("SELECT  FROM interface_request WHERE RequestCode in (");
			interfaceRequest.append("SELECT RequestCode FROM temp_animal_quarantine ");
			interfaceRequest.append("WHERE DocumentName = '" + documentName + "' AND DocumentYear = '" + documentYear + "'");
			interfaceRequest.append(")");
			interfaceRequest.append(") interMax ON inter.RequestCode = interMax.RequestCode");*/
			
			//interfaceRequest.append("select RequestCode from interface_request ");
			//interfaceRequest.append("where RequestCode in (select RequestCode from temp_animal_quarantine where DocumentName = '" + documentName + "' and DocumentYear = '" + documentYear + "') ");
			//interfaceRequest.append("and RequestedDate = ( ");
			//interfaceRequest.append("select max(RequestedDate) from interface_request where RequestCode in (select RequestCode from temp_animal_quarantine where DocumentName = '" + documentName + "' and DocumentYear = '" + documentYear + "')");
			//interfaceRequest.append(")");
	...
	public List<TempAnimalQuarantine> findByDocumentNameAndDocumentYearOrderByDescRequestDate(long documentName, int documentYear) {
		...
		try {
			...
			query.append("SELECT * FROM temp_animal_quarantine WHERE DocumentName = ? and DocumentYear = ? order by ID asc");
